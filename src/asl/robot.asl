...
+!receive(money) : pay(money, Qtd)[source(owner)] & not noMoneyLeftToday <-
...
+!receive(money) : not pay(money, Qtd)[source(owner)] & not noMoneyLeftToday <-
	.println("Estoy esperando a que Owner me pague");
	.wait(1000);
	!receive(money). //TODO consider delete this line if "extravío" is a possibility.
+!receive(money) : noMoneyLeftToday <-
	.println("El presupuesto de Owner para hoy se ha agotado").
	-noMoneyLeftToday.
