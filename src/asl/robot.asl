...
limit(min, fridge,   beer,  3 ). //M韓imo de cervezas que deber韆 haber en el frigo, si hay menos se ordenan m醩
limit(max, dumpster, trash, 3 ).
limit(max, owner,    beer,  2).
limit(min, buy,      beer,  3 ). //Cantidad de cervezas a pedirle al s鷓er (en cada orden)
...
+pay(Amount)[source(owner)] <-
	.println("Gracias por la paga de ", Amount, " mi se帽or");
	?has(money, Balance);
	.abolish(has(money, _));
...
+!dialogWithOwner : msg(Msg)[source(Ag)] & bot(Bot) <-
	...
	//chat(Msg) // De manera as铆ncrona devuelve una signal => answer(Answer)
	-msg(Msg)[source(Ag)];   
...
+!doSomething(Answer,Ag) : service(Answer, Service) <-
	.println("Aqui debe ir el c贸digo del servicio:", Service," para el agente ",Ag).
...
+!cleanHouse : requestedRetrieval(can, floor(X, Y)) & not cleaning(_, can, floor(X, Y)) <-
	.println("Owner ha tirado una lata al suelo, activo un aut贸mata para que limpie");
...
+!cleanHouse : requestedRetrieval(can, owner) & not cleaning(_, can, owner) <-
	.println("Owner me ha pedido que vaya a recoger una lata, activo un aut贸mata para que la recoja");
...
+!cleanHouse : overLimit(max, trash, dumpster) & not takingout(_, trash) <-
	.println("El dumpster est谩 lleno, activo un aut贸mata para sacar la basura");
...
+!manageBeer : asked(Ag, beer) & available(beer, fridge) & not moving(_, beer, 1, fridge, Ag) & not healthConstraint(beer, Ag, _) <-
	.println(Ag, " me ha pedido un ", "beer", ", activo un aut贸mata para que se lo lleve");
...
+!manageBeer : not overLimit(min, beer, fridge) & not ordered(beer) & cheapest(Provider, beer, Price) <-
	.println("Tengo menos cerveza de la que deber铆a, voy a comprar m谩s");
...
+!manageBeer : asked(Ag, beer) & healthConstraint(beer, Ag, Msg) <-
	.println(Ag, " no puede beber m谩s ", "beer");
...
+moved(failure, Product, Qtty, Origin, Destination)[source(Mover)] <-
	...
	if (Destination == owner) {
		.send(Destination, tell, msg("No me queda, voy a comprar m谩s"));
	}
	.abolish(moving(Mover, Product, Qtty, Origin, Destination));
